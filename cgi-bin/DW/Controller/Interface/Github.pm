...
sub _labels_to_keep {
    ...
    if ( $claim_status && none { $_ eq $claim_status } @current_labels ) {
...
sub _filter_autogenerated_labels {
    return grep { $_ ne "status: claimed" } @_;
}
...
sub label_from_comment {
    ...
    my $current_status = $payload->{issue}->{assignee} ? "status: claimed" : undef;

    # if we're commenting on an issue (not a pull request), we may want to update the claimed status
